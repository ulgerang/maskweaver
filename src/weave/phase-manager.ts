    ...
    for (const phase of plan.phases) {
        ...
        for (const task of phase.tasks) {
            ...
            if (task.dependsOn && task.dependsOn.length > 0) {
                lines.push(`        depends_on: [${task.dependsOn.map(dep => yamlEscapeString(dep)).join(', ')}]`);
            }
            if (task.verify && task.verify.length > 0) {
                lines.push('        verify:');
                for (const verifyItem of task.verify) {
                    lines.push('          - kind: ' + yamlEscapeString(verifyItem.kind));
                    lines.push('            value: ' + yamlEscapeString(verifyItem.value));
                }
            }
