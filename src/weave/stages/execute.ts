...
function sanitizeNodeToken(nodeId: string): string {
    return nodeId
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '')
...
async function collectGdcExtractContexts(
    ...
    for (const task of phase.tasks) {
        ...
        for (const nodeId of nodeIds) {
            ...
            if (gdc.extractContext.withTests) args.push('--with-tests');
            if (gdc.extractContext.withCallers) args.push('--with-callers');
            args.push('--output', outputPathRel);
...
export function formatExecutionPlan(plan: PhaseExecutionPlan): string {
    ...
    if (plan.gdcContextFiles && plan.gdcContextFiles.length > 0) {
        const generated = plan.gdcContextFiles.filter(item => item.status === 'generated');
        const failed = plan.gdcContextFiles.filter(item => item.status === 'failed');

        lines.push('### GDC Context Files');
        lines.push('');
        ...
        if (failed.length > 0) {
            ...
            for (const item of failed.slice(0, 10)) {
                lines.push(`- [${item.taskId}] \`${item.nodeId}\`: ${item.note || 'extract failed'}`);
            }
        }
    ...
    lines.push('Use `weave craft` to refresh execution context while implementing.');
    lines.push('');
    lines.push('**Suggested execution flow:**');
    lines.push(`1) Run: \`weave command=craft phaseId="${plan.phaseId}"\``);
    lines.push('2) Implement/delegate the planned work with the selected agent tier + mask');
    lines.push('3) Verify with `weave command=verify` (or project-specific build/test commands)');
    lines.push(`4) Finalize with: \`weave command=approve-plan phaseId="${plan.phaseId}"\``);
    lines.push('');
    lines.push('If finalization fails, fix issues and rerun verification before approval.');
    lines.push('');
