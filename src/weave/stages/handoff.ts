...
function collectPhaseNodeIds(phase: WeavePhase): string[] {
...
async function buildGdcCompletionSummary(basePath: string | undefined, phase: WeavePhase): Promise<string[]> {
    const runtimeBase = basePath || process.cwd();
    const gdc = getEffectiveGdcConfig(runtimeBase);
    if (!gdc.enabled) return [];

    const nodeIds = collectPhaseNodeIds(phase);
    const lines: string[] = [];
    lines.push('');
    lines.push('### GDC Node Status');
    lines.push(`- Related nodes: ${nodeIds.length > 0 ? nodeIds.map(nodeId => `\`${nodeId}\``).join(', ') : 'none'}`);
    ...
    if (typeof stats.total === 'number') {
        lines.push(`- Graph stats: total=${stats.total}, implemented=${stats.implemented ?? 0}, tested=${stats.tested ?? 0}`);
    } else {
        lines.push(`- Graph stats unavailable (stats exit=${statsResult.exitCode})`);
    }
...
export async function handleUserResponse(
    ...
    switch (response) {
        case 'approve': {
            ...
            const gdcLines = await buildGdcCompletionSummary(basePath, phase);
            if (gdcLines.length > 0) {
                message += `${gdcLines.join('\n')}\n\n`;
            }
