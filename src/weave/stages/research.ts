...
type WorkspaceContextReport = {
    ...
    afterContext: string[];
    gdc: GdcContextReport;
};

type GdcContextReport = {
    detected: boolean;
    gateEnabled: boolean;
    configPath?: string;
    nodeSpecFiles: number;
    hasGraphDb: boolean;
    sampleNodes: string[];
    machineSignals: string[];
    candidateReuseNodes: string[];
    dependencyBlastRadius: string[];
    driftSignals: string[];
};
    ...
    while (stack.length > 0 && results.length < maxFiles) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (entry.name.startsWith('.') && !WORKSPACE_ALLOWED_DOT_DIRS.has(entry.name)) continue;
                stack.push(fullPath);
                continue;
            }
...
function listFilesRecursive(rootDir: string, exts?: Set<string>): string[] {
    const files: string[] = [];
...
function containsNeedle(target: string, needles: string[]): boolean {
    const lower = target.toLowerCase();
    return needles.some(needle => lower.includes(needle));
}

async function analyzeGdcContext(basePath: string, intake: IntakeResult): Promise<GdcContextReport> {
    ...
    let statsResult: Awaited<ReturnType<typeof runGdcMachineCommand>> | null = null;
    let graphResult: Awaited<ReturnType<typeof runGdcMachineCommand>> | null = null;
    let checkResult: Awaited<ReturnType<typeof runGdcMachineCommand>> | null = null;
    ...
    if (!hasConfig) driftSignals.push('`.gdc/config.yaml` missing: spec/runtime alignment checks may be incomplete.');
    if (!hasGraphDb) driftSignals.push('`.gdc/graph.db` missing: run `gdc sync` before `gdc check`.');
    if (nodeSpecFiles.length === 0) driftSignals.push('No `.gdc/nodes/*.yaml` specs found.');
...
async function analyzeWorkspaceContext(basePath: string, intake: IntakeResult): Promise<WorkspaceContextReport> {
...
export async function buildResearchReport(options: ResearchOptions): Promise<string> {
